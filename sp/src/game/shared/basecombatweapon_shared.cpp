...
{
	return GetWpnData().bUseIronsight;
}
...
const char *CBaseCombatWeapon::GetDualWieldClass(void) const
{
	return GetWpnData().szDualWieldClass;
}

bool CBaseCombatWeapon::CanDualWield(void) const
{
	return GetWpnData().bCanDualWield;
}

void CBaseCombatWeapon::DualWield(CBasePlayer *pOwner)
{
	if (!pOwner)
		return;
	...
	CBaseCombatWeapon *pNewWeapon = pOwner->Weapon_OwnsThisType(GetDualWieldClass());

	if (pNewWeapon)
	{
		pOwner->Weapon_Switch(pNewWeapon);
	}
}
