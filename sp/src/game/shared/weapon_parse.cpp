...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	bUseIronsight = (pKeyValuesData->GetInt("UseIronsights", 0) != 0) ? true : false;

	bCanDualWield = (pKeyValuesData->GetInt("CanDualWield", 0) != 0) ? true : false;
	Q_strncpy( szDualWieldClass, pKeyValuesData->GetString( "DualWieldClass"), MAX_WEAPON_STRING );
