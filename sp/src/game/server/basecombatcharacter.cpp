...
ConVar	ai_serverragdoll_twitch("ai_serverragdoll_twitch", "1", FCVAR_ARCHIVE);
...
bool CBaseCombatCharacter::CorpseGib( const CTakeDamageInfo &info )
{
	...
	//we don't need this because of our new gib system. but we still need to gib shit.
	/*
	if ( HasHumanGibs() )
	{
...
bool CBaseCombatCharacter::BecomeRagdollBoogie( CBaseEntity *pKiller, const Vector &forceVector, float duration, int flags, float mag)
{
	...
	CRagdollBoogie::Create( pRagdoll, mag, gpGlobals->curtime, duration, flags );
...
// UNDONE: Put in a real sound cue here, don't do this bogus hack anymore
	...
	if (ai_serverragdoll_twitch.GetBool())
	{
		BecomeRagdollBoogie(newinfo.GetAttacker(), forceVector, 2.0f, 0, 100.0f);
		return true;
	}
	else
	{
