...
#define GAUSS_RECHARGE_TIME 0.35f
...
void CWeaponGaussGun::Fire( void )
{
	...
	CTakeDamageInfo dmgInfo(this, pOwner, sk_plr_dmg_gauss.GetFloat(), DMG_SHOCK | DMG_SNIPER);
...
void CWeaponGaussGun::ChargedFire( void )
{
	...
	{
		CTakeDamageInfo dmgInfo(this, pOwner, flDamage, DMG_SHOCK | DMG_ALWAYSGIB);
		CalculateBulletDamageForce( &dmgInfo, m_iPrimaryAmmoType, aimDir, tr.endpos );
...
void CWeaponGaussGun::ItemPostFrame(void)
{
	...
	if (((pPlayer->m_nButtons & IN_ATTACK) == false) && ((pPlayer->m_nButtons & IN_ATTACK2) == false) && (m_iClip1 < sk_max_gauss_round.GetInt()))
	{
		RechargeAmmo();
	}
...
void CWeaponGaussGun::RechargeAmmo(void)
{
	...
	m_flNextCharge = gpGlobals->curtime + GAUSS_RECHARGE_TIME;
}
