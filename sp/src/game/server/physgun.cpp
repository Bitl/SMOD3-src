...
#define PHYSGUN_BEAM_SPRITE		"sprites/physbeam.vmt" //Лучик физгана

#define MAX_PELLETS	16 //Кол-во пеллетов(патронов) для чуда

class CWeaponGravityGun; //Класс пеллета
...
public:
	...
	void Precache()
	{
		SetModelName( MAKE_STRING( "models/weapons/glue.mdl" ) ); //Моделька для пелетов
...
LINK_ENTITY_TO_CLASS(gravity_pellet, CGravityPellet); //регистрируем пелеты
...
BEGIN_DATADESC( CGravityPellet )
	...
	//физическая система запустит этот Input, если constraint будет остановлен из-за физики
...
class CWeaponGravityGun : public CBaseHLCombatWeapon //класс самого физгана
{
	DECLARE_DATADESC();
...
public:
	...
	CWeaponGravityGun();
	void Spawn( void ); //появление
	void OnRestore( void );
	void Precache( void ); //подгрузка

	void PrimaryAttack( void ); //первичная(основная атака)
	void SecondaryAttack( void );//вторичная атака
	void WeaponIdle( void );//движение оружия(анимация)
	...
	void SoundCreate( void );//создание звука
	void SoundDestroy( void );//уничтожение звука
	void SoundStop( void );//остановка звука
	void SoundStart( void );//начало звука
	void SoundUpdate( void );//обновление звука
	void AddPellet( CGravityPellet *pPellet, CBaseEntity *pParent, const Vector &surfaceNormal );
	void DeleteActivePellets();//удаление активных пелетов
...
LINK_ENTITY_TO_CLASS( weapon_physgun, CWeaponGravityGun );//регистрация физгана
...
void CWeaponGravityGun::EffectUpdate( void )
{
	...
	if ( tr.fraction != 1 )
	{
		...
		// если предмет находится слишком близко к игроку, то бросаем его
		if ( distance < 36 )
		{
			DetachObject();
			return;
	...
	if ( m_hObject == NULL && tr.DidHitNonWorldEntity() )
	{
		...
		// информируем о том, что обьект стукнулся
		ClearMultiDamage();
	...
	if ( best < 3*3 )
				{
					...
					//Останавливаем тягу пеллета, если объект уже касается
		m_glueTouching = true;
		}
	}
...
void CWeaponGravityGun::SecondaryAttack( void )
{
	...
	m_viewModelIndex = pOwner->entindex();
	// Make sure I've got a view model
	//удостоверимся, что есть моделька
	CBaseViewModel *vm = pOwner->GetViewModel();
	if ( vm )
	{
		m_viewModelIndex = vm->entindex();
	...
	else
	{
		...
		//если обьект не физический или является брашем, то не свариваем
...
void CWeaponGravityGun::WeaponIdle( void )
{
	if ( HasWeaponIdleTimeElapsed() )
	{
		...
		if ( m_active )
		{
			...
			//если пелет соприкасается с обьектом, значит свариваем!
...
{
	//Напоминаем, что у нас есть патроны
	return true;
}
