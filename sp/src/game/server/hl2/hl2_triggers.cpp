...
extern ConVar sv_player_equipfists;
...
void CTriggerWeaponStrip::StartTouch(CBaseEntity *pOther)
{
	...
	if ( pCharacter && pCharacter->IsAllowedToPickupWeapons() )
	{
		...
		//HACK: Give the player back his fists after dropping all the weapons.
		CHL2_Player *pPlayer = dynamic_cast<CHL2_Player*>(pCharacter);
		if (pPlayer->IsSuitEquipped())
		{
			if (sv_player_equipfists.GetFloat())
			{
				if (!pPlayer->Weapon_OwnsThisType("weapon_fists"))
				{
