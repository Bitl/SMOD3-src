...
ConVar sk_kick_dmg_div("sk_kick_dmg_div", "3");
ConVar sk_kick_propdmg_div("sk_kick_propdmg_div", "10");
ConVar sk_kick_shake_propmass("sk_kick_shake_propmass", "150");
ConVar sk_kick_dmg_div_citizen("sk_kick_dmg_div_citizen", "6");

ConVar sv_player_legacyflashlight("sv_player_legacyflashlight", "0", FCVAR_ARCHIVE);
ConVar sv_player_equipfists("sv_player_equipfists", "1", FCVAR_ARCHIVE);
...
bool Flashlight_UseLegacyVersion( void )
{
	...
	*/
	...
	return sv_player_legacyflashlight.GetBool();
}
...
void CHL2_Player::Precache( void )
{
	...
	PrecacheModel("models/weapons/v_kick_citizen.mdl");
...
void CHL2_Player::EquipSuit( bool bPlayEffects )
{
	...
	if ( bPlayEffects == true )
	{
		...
		if (sv_player_equipfists.GetFloat())
		{
			if (!Weapon_OwnsThisType("weapon_fists"))
			{
...
void CHL2_Player::CheckKickModel(void)
{
	...
	{
		Leg->SetWeaponModel("models/weapons/v_kick_citizen.mdl", NULL);
	}
}
...
void CHL2_Player::KickAttack(void)
{
	...
	if (!IsDead())
	{
		...
		if (vm)
		{
			...
			QAngle	recoil = QAngle(random->RandomFloat(2.0f, 8.0f), random->RandomFloat(-8.0f, 8.0f), 0);
			...
			float KickDamageMult;
			if (IsSuitEquipped())
			{
				KickDamageMult = KickThrowForceMult / sk_kick_dmg_div.GetFloat();
			}
			else
			{
				KickDamageMult = KickThrowForceMult / sk_kick_dmg_div_citizen.GetFloat();
			}
				...
				else
				{
					...
					*/
					...
					if (pProp && !pProp->IsNPC())
					{
						...
						{
							UTIL_ScreenShake(this->WorldSpaceCenter(), 25.0, 150.0, 1.0, 750, SHAKE_START);
						}
					...
					if (Victim && Victim->IsNPC())
					{
						...
						{
							CTakeDamageInfo info(this, this, Victim->GetHealth(), DMG_CRUSH);
...
{
	// if we are not moving and bullettime is on, DON'T RECHARGE.
	if (!fabs(GetAbsVelocity().x) && !fabs(GetAbsVelocity().y) && SuitPower_IsDeviceActive(SuitDeviceBulletTime))
		return false;
...
void CHL2_Player::UpdateWeaponPosture( void )
{
	...
	if ( pWeapon && m_LowerWeaponTimer.Expired() && pWeapon->CanLower() )
	{
		...
		if (  aimTarget && !tr.DidHitWorld() )
		{
			...
			//See if we hates it
			if (aimTarget->IsNPC() && aimTarget->MyNPCPointer()->IsPlayerAlly())
			{
