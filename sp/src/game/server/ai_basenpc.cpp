...
struct ParticleForHeadshot_t
{
	int nColor;
	const char *lpszParticleSystemName;
};

ParticleForHeadshot_t bloodHeadshotCallbacks[] =
{
	{ BLOOD_COLOR_RED, "smod_headshot_r" },
	{ BLOOD_COLOR_GREEN, "smod_headshot_g" },
	{ BLOOD_COLOR_YELLOW, "smod_headshot_y" },
#if defined( HL2_EPISODIC )
	{ BLOOD_COLOR_ANTLION, "smod_headshot_y" },		// FIXME: Move to Base HL2
	{ BLOOD_COLOR_ZOMBIE, "smod_headshot_y" },			// FIXME: Move to Base HL2
	{ BLOOD_COLOR_ANTLION_WORKER, "smod_headshot_g" },
#endif // HL2_EPISODIC
};

ParticleForHeadshot_t bloodDecapitationCallbacks[] =
{
	{ BLOOD_COLOR_RED, "smod_blood_decap_r" },
	{ BLOOD_COLOR_GREEN, "smod_blood_decap_g" },
	{ BLOOD_COLOR_YELLOW, "smod_blood_decap_y" },
#if defined( HL2_EPISODIC )
	{ BLOOD_COLOR_ANTLION, "smod_blood_decap_y" },		// FIXME: Move to Base HL2
	{ BLOOD_COLOR_ZOMBIE, "smod_blood_decap_y" },			// FIXME: Move to Base HL2
	{ BLOOD_COLOR_ANTLION_WORKER, "smod_blood_decap_g" },
#endif // HL2_EPISODIC
};

ParticleForHeadshot_t bloodGibCallbacks[] =
{
	{ BLOOD_COLOR_RED, "smod_blood_gib_r" },
	{ BLOOD_COLOR_GREEN, "smod_blood_gib_g" },
	{ BLOOD_COLOR_YELLOW, "smod_blood_gib_y" },
#if defined( HL2_EPISODIC )
	{ BLOOD_COLOR_ANTLION, "smod_blood_gib_y" },		// FIXME: Move to Base HL2
	{ BLOOD_COLOR_ZOMBIE, "smod_blood_gib_y" },			// FIXME: Move to Base HL2
	{ BLOOD_COLOR_ANTLION_WORKER, "smod_blood_gib_g" },
#endif // HL2_EPISODIC
};
...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if (ShouldGib(info) && CanUseNewGoreSystem())
	{
		GoreEffect(info);
	}
...
bool CAI_BaseNPC::ShouldHeadshot(const CTakeDamageInfo &info)
{
	if (!CanHeadshot())
		return false;
...
#ifdef HL2_DLL
	if (info.GetDamageType() & (DMG_SNIPER | DMG_BUCKSHOT | DMG_SLASH))
		return true;
#else
	if (info.GetDamageType() & (DMG_BUCKSHOT | DMG_SLASH))
		return true;
#endif
...
int CAI_BaseNPC::GetHeadshotDamageType(const CTakeDamageInfo &info)
{
#ifdef HL2_DLL
	if (info.GetDamageType() & (DMG_SNIPER | DMG_BUCKSHOT))
		return HEADSHOT_DISMEMBERMENT;
#else
	...
	if (info.GetDamageType() & (DMG_SLASH))
		return HEADSHOT_DECAPITATION;

	return HEADSHOT_DISMEMBERMENT;
}

void CAI_BaseNPC::HeadshotEffect(const CTakeDamageInfo &info, int headshotMode)
{
	// for some reason this wasn't included. AAAA. -Bitl
	if (!ShouldHeadshot(info))
		return;

	SetModel(GetGibName_Beheaded());

	if (headshotMode == HEADSHOT_DECAPITATION)
	{
		for (int i = 0; i < ARRAYSIZE(bloodDecapitationCallbacks); i++)
		{
			if (bloodDecapitationCallbacks[i].nColor == BloodColor())
			{
				DispatchParticleEffect(bloodDecapitationCallbacks[i].lpszParticleSystemName, PATTACH_POINT_FOLLOW, this, "bloodspurt", true);
				break;
			}
		}
		...
		CGib::SpawnSpecificGibs(this, 1, 150, 450, GetGibName_Head(), 6);
		CGib::SpawnSpecificStickyGibs(this, 3, 150, 450, "models/gibs/pgib_p4.mdl", 6);
	}
	else if (headshotMode == HEADSHOT_DISMEMBERMENT)
	{
		for (int i = 0; i < ARRAYSIZE(bloodHeadshotCallbacks); i++)
		{
			if (bloodHeadshotCallbacks[i].nColor == BloodColor())
			{
				DispatchParticleEffect(bloodHeadshotCallbacks[i].lpszParticleSystemName, PATTACH_POINT_FOLLOW, this, "bloodspurt", true);
				break;
			}
		}
	...
	EmitSound("Gore.Splash");
	m_iHealth = 0;
	Event_Killed(info);
}

void CAI_BaseNPC::GoreEffect(const CTakeDamageInfo &info)
{
	if (!CanUseNewGoreSystem())
		return;
	...
	for (int i = 0; i < ARRAYSIZE(bloodGibCallbacks); i++)
	{
		if (bloodGibCallbacks[i].nColor == BloodColor())
		{
			DispatchParticleEffect(bloodGibCallbacks[i].lpszParticleSystemName, GetAbsOrigin(), GetAbsAngles(), this);
			break;
		}
	}
	EmitSound("Gore.Splash");

	CGib::SpawnSpecificGibs(this, 1, 50, 100, GetGibName_Head(), 6);
	...
	CBaseEntity *pLeftArmGib = CreateRagGib(this, GetGibName_Arm_L(), GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, 6, IsOnFire());
	if (pLeftArmGib)
	{
	...
	CBaseEntity *pRightArmGib = CreateRagGib(this, GetGibName_Arm_R(), GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, 6, IsOnFire());
	if (pRightArmGib)
	{
	...
	CBaseEntity *pTorsoGib = CreateRagGib(this, GetGibName_Torso(), GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, 6, IsOnFire());
	if (pTorsoGib)
	{
	...
	CBaseEntity *pPelvisGib = CreateRagGib(this, GetGibName_Pelvis(), GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, 6, IsOnFire());
	if (pPelvisGib)
	{
	...
	CBaseEntity *pLeftLegGib = CreateRagGib(this, GetGibName_Leg_L(), GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, 6, IsOnFire());
	if (pLeftLegGib)
	{
	...
	CBaseEntity *pRightLegGib = CreateRagGib(this, GetGibName_Leg_R(), GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, 6, IsOnFire());
	if (pRightLegGib)
	{
...
void CAI_BaseNPC::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	case HITGROUP_HEAD:
		if (ShouldHeadshot(info) || HeadshotRandom == 0)
		{
			HeadshotEffect(info, GetHeadshotDamageType(info));
		}
		else
		{
	...
	default:
		if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) && (pEvent->type & AE_TYPE_SERVER))
		{
			...
			else if ( pEvent->event == AE_NPC_RAGDOLL )
			{
				...
				CTakeDamageInfo newinfo = CTakeDamageInfo(this, this, 0, DMG_GENERIC);
				BecomeRagdoll(newinfo, vec3_origin);
				return;
			}
...
void CAI_BaseNPC::Precache( void )
{
	...
	PrecacheScriptSound( "Gore.Splash" );
	...
	if (CanUseNewGoreSystem())
	{
		PrecacheModel(GetGibName_Head());
		PrecacheModel(GetGibName_Torso());
		PrecacheModel(GetGibName_Pelvis());
		PrecacheModel(GetGibName_Arm_L());
		PrecacheModel(GetGibName_Arm_R());
		PrecacheModel(GetGibName_Leg_L());
		PrecacheModel(GetGibName_Leg_R());
		PrecacheModel(GetGibName_Beheaded());
	}
	else if (CanHeadshot())
	{
		PrecacheModel(GetGibName_Head());
...
{ 
	...
	if (Classify() == CLASS_CITIZEN_PASSIVE)
		return true;
	if (Classify() == CLASS_CITIZEN_REBEL)
		return true;
