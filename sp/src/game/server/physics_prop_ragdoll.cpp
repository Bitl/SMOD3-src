...
void CRagdollProp::Precache( void )
{
	...
	PrecacheModel("models/gibs/soldier_shotgun_prisonguard_head.mdl");
...
int	CRagdollProp::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if (info.GetDamageType() & (DMG_SLASH | DMG_CRUSH | DMG_BLAST))
	{
		if (CreateGibsForModel())
		{
			RemoveDeferred();
		}
...
bool CRagdollProp::CreateGibsForModel()
{
	const char *modelname = STRING(GetModelName());

	if (Q_stristr(modelname, "models/combine_soldier.mdl"))
	{
		if (m_nSkin == 1)
		{
		}
	...
	else if (Q_stristr(modelname, "models/combine_super_soldier.mdl"))
	{
		return true;
	}
	else if (Q_stristr(modelname, "models/combine_soldier_prisonguard.mdl"))
	{
		if (m_nSkin == 1)
		{
		}
	...
	else if (Q_stristr(modelname, "models/police.mdl") || Q_stristr(modelname, "models/police_cheaple.mdl"))
	{
		return true;
	}
	if (Q_stristr(modelname, "models/gibs/combine_soldier_beheaded.mdl"))
	{
		return true;
	}
	else if (Q_stristr(modelname, "models/gibs/combine_super_soldier_beheaded.mdl"))
	{
		return true;
	}
	else if (Q_stristr(modelname, "models/gibs/combine_prisonguard_beheaded.mdl"))
	{
		return true;
	}
	else if (Q_stristr(modelname, "models/gibs/combine_shotgunner_beheaded.mdl"))
	{
		return true;
	}
	else if (Q_stristr(modelname, "models/gibs/police_beheaded.mdl"))
	{
		return true;
	}
	else if (Q_stristr(modelname, "models/antlion.mdl"))
	{
		return true;
	}
	else if (Q_stristr(modelname, "models/antlion_worker.mdl"))
	{
		return true;
	}
...
void CRagdollProp::TraceAttack( const CTakeDamageInfo &info, const Vector &dir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	SpawnBlood(ptr->endpos, dir, BloodColor(), info.GetDamage());// a little surface blood.
	TraceBleed(info.GetDamage(), dir, ptr, info.GetDamageType());
