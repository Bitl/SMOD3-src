...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 101:
		...
		GiveAmmo( 8,	"CGuard" );
		GiveAmmo(30, "GaussEnergy");
		GiveNamedItem( "weapon_smg1" );
		GiveNamedItem( "weapon_frag" );
		GiveNamedItem( "weapon_crowbar" );
		GiveNamedItem( "weapon_pistol" );
		GiveNamedItem("weapon_dual_pistol");
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	else if (stricmp(cmd, "dual_wield") == 0)
	{
		CBaseCombatWeapon *pWeapon = GetActiveWeapon();
		if (pWeapon != NULL)
			pWeapon->DualWield(this);
...
bool CBasePlayer::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	if (Weapon_OwnsThisType( pWeapon->GetClassname(), pWeapon->GetSubType())) 
	{
		...
		{
			if (!Weapon_OwnsThisType(pWeapon->GetDualWieldClass(), pWeapon->GetSubType()))
			{
				GiveNamedItem(pWeapon->GetDualWieldClass());
				UTIL_Remove(pWeapon);
				return true;
			}
			else
